<template>
  <li :class="['sidebar__item', props.route.currentPage ? props.activeClass + '_parent' : '']">
    <router-link 
      :to="getRoutePath(props.route.href)"
      :class="['heading heading--4 heading--white w-100 p-y-8 p-x-10', props.route.currentPage ? props.activeClass : '']"
    >
      {{ props.route.linkName }}
      <span v-if="props.route.nested && !!props.route.nested.length"><IconDropdownArrow /></span>
    </router-link>
    <ul
      v-if="props.route.nested && !!props.route.nested.length && props.route.currentPage"
      class="sidebar__list"
    >
