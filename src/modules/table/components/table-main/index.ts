...
export default defineComponent({
  ...
  setup (props, context) {
    const tableHead = ref<IHeadItemBase[] | IHeadItemGrouped[]>(props.tableHead),
    ...
    const hasChildren = computed<boolean>(() => {
      if (
        tableHead.value.some((item: IHeadItemGrouped | IHeadItemBase) => item.children && !!item.children.length)
      ) {
        return true
      }
    ...
    const tableColumnDataHead: Ref<IHeadItemBase[] | IHeadItemGrouped[]> = computed(() => hasChildren.value ? tableHeadGrouped.value : tableHead.value) as Ref<IHeadItemBase[] | IHeadItemGrouped[]>
